...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<style type="text/css">
		...
		#content tr td {height: 60px;padding-top: 5px;font-size: 13px;border-bottom: 1px solid #ddd;
  ...
  <body>
    <div id="mainContent">
		...
		<div id="searchDiv">
			...
			<form id="searchForm" action="${pageContext.request.contextPath}/order/self" method="post">
				...
				<c:if test="${list.size() > 0 }">
					<c:forEach items="${list }" var="o" varStatus="st">
						<tr>
							...
							<td width="10%" class="word" title="${o.goodsName }">
								<span class="word">
									<c:if test="${o.orderType == '1'}">
										<span style="color: #f66;font-weight: bold;cursor: pointer;">${o.goodsName }</span>
									</c:if>
									<c:if test="${o.orderType == '2'}">托管</c:if>
									<c:if test="${o.orderType == '3'}">护理</c:if>
								</span>
							</td>
							<td width="5%"><span class="word">${o.unitPrice }</span></td>
							<td width="5%"><span class="word">${o.amount }</span></td>
							<td width="10%"><span class="word">${o.totalPrice }</span></td>
							<td width="15%"><span class="word"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${o.checkTime }"/></span></td>
							<td width="20%">
								<c:choose>
									<c:when test="${o.orderType == '2'}">
										托管：<span style="color: #f33;">${o.amount }</span>&nbsp;天<br>
										<span>开始日期：<fmt:formatDate pattern="yyyy-MM-dd" value="${o.tyDate }"/></span><br>
									</c:when>
									<c:when test="${o.orderType == '3'}">
										护理：<span style="color: #f33;">${o.amount }</span>&nbsp;次<br>
