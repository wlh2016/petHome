...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <div id="mainContent">
		<div id="vLocation">
			当前位置：饰品管理 / <a href="javascript:void(0)" style="color: #0088cc;">饰品列表</a>
		</div>
		
		<div id="searchDiv">
			<div id="search">Search&nbsp;&nbsp;</div>
			<form id="searchForm" action="${pageContext.request.contextPath}/ornament/list" method="post">
				<input id="searchName" name="name" type="text" placeholder="请输入饰品名称" style="height: 24px;" value="${name }"/>
				<input type="submit" value="搜索" style="margin-left: 15px;cursor: pointer;"/>
				<a href="${pageContext.request.contextPath}/ornament/add"><input type="button" value="新增" style="margin-left: 15px;cursor: pointer;"/></a>
			 </form>
		</div>
		...
		<table id="content" class="t" cellpadding="0" cellspacing="0">
				<c:if test="${list.size() == 0 }">
					<tr><td style="color: red;font-weight: bold;width: 100%;line-height: 80px;">未查询到饰品信息</td></tr>
				</c:if>
				<c:if test="${list.size() > 0 }">
					<c:forEach items="${list }" var="f" varStatus="st">
						<tr>
							...
							<td width="10%"><span class="word">${f.unitPrice }</span></td>
							<td width="10%" title="${f.productionMaterial }"><span class="word">${f.productionMaterial }</span></td>
							...
							<td width="9%"><span title="${f.note }" class="word" style="cursor: pointer;">${f.note }</span></td>
							<td width="15%">
								<span class="word">
									<a href="${pageContext.request.contextPath}/ornament/edit/${f.id }" title="修改饰品信息">
										<img src="${pageContext.request.contextPath}/style/images/edit.png" border="0" style="CURSOR: hand;margin-top: -8px;">
									</a>
								</span>
								<span class="word">
									<a onclick="deletePet(${f.id})" title="删除饰品" style="margin-left: 20px;">
  ...
  <script type="text/javascript">
	...
	function deletePet(id) {
		...
		if(cfm){
			...
			$.ajax({
				url: '${pageContext.request.contextPath}/ornament/delete',
				type: 'post',
				data: {id: id},
