...
<html>
  ...
  <body>
	<div id="vLocation">
		当前位置&nbsp;：饰品管理&nbsp;&nbsp;<span class="divider">/</span>&nbsp;&nbsp;<a href="javascript:void(0)">修改饰品</a>
	</div>
	<div id="infoDiv">
		<form id="saveForm" action="${pageContext.request.contextPath}/ornament/edit" method="post" enctype="multipart/form-data">
			<input type="hidden" name="id" value="${o.id }"/>
			<input type="hidden" name="goodType" value="${o.goodType }"/>
			<input type="hidden" name="sid" value="${stock.id }"/>
			<table id="infoTable">
				<tr>
					<td class="leftTD">名称</td>
					<td class="rightTD"><input type="text" id="name" name="name" value="${o.name }"/></td>
				</tr>
				<tr>
					<td class="leftTD">生产日期</td>
					<td class="rightTD"><input type="date" id="productionDateT" value="${o.productionDate }" name="productionDateT" required="required"/></td>
				</tr>
				<tr>
					<td class="leftTD">材质</td>
					<td class="rightTD"><input type="text" id="productionMaterial" value="${o.productionMaterial }" name="productionMaterial" required="required"/></td>
				</tr>
				<tr>
					<td class="leftTD">单价</td>
					<td class="rightTD"><input type="text" id="unitPrice" name="unitPrice" value="${o.unitPrice }"/>&nbsp;元</td>
				</tr>
				<tr>
					<td class="leftTD">介绍</td>
					<td class="rightTD"><input type="text" id="introduction" name="introduction" value="${o.introduction }" required="required"/></td>
				</tr>
				<tr>
					<td class="leftTD">备注</td>
					<td class="rightTD">
						<textarea id="note" rows="5" cols="20" name="note">${o.note }</textarea>
					</td>
				</tr>
				<tr>
					<td class="leftTD">图片</td>
					<td class="rightTD"><input type="file" id="file" name="file" required="required" src="${o.picLocation }"/></td>
				</tr>
				<tr>
					<td class="leftTD" style="color: #ccc;">预览</td>
					<td class="rightTD">
						<img id="ImgPr" width="180" height="100" src="${pageContext.request.contextPath}/${o.picLocation }"/>
					</td>
				</tr>
				<tr>
  ...
  <script type="text/javascript">
  	$(function(){
  		...
  		$("#subBtn").click(function() {
			...
			var  productionMaterial = $("#productionMaterial").val();
			...
			if (name!='' && unitPrice!='' && productionDateT!='' && productionMaterial!='' && weight!='' && introduction!='' && note!='' && storeNum!='' && areaNum!='' && placeNum!='' && amount!='') {
				if (file == '') {
					alert("请选择饰品图片");
					return false;
				} else {
