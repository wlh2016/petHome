...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <div id="mainContent">
		<div id="vLocation">
			当前位置：购物车管理 / <a href="javascript:void(0)" style="color: #0088cc;">购物车列表</a>
		</div>
		...
		<table id="title" class="t" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td width="20%">图片</td>
				<td width="15%">名称</td>
				<td width="15%">单价(RMB)</td>
				<td width="15%">数量</td>
				<td width="15%">总价(RMB)</td>
				<td width="15%">操作</td>
			</tr>
		</table>
		...
		<table id="content" class="t" cellpadding="0" cellspacing="0">
				<c:if test="${list.size() == 0 }">
					<tr><td style="color: red;font-weight: bold;width: 100%;line-height: 80px;">未查询到购物车记录</td></tr>
				</c:if>
				<c:if test="${list.size() > 0 }">
					<c:forEach items="${list }" var="c" varStatus="st">
						<tr>
							...
							<td width="20%">
								<img alt="" src="${pageContext.request.contextPath}/${c.picLocation }" class="pic">
							</td>
							<td width="15%" class="word"><span class="word">${c.goodsName }</span></td>
							<td width="15%"><span class="word">${c.unitPrice }</span></td>
							<td width="15%"><span class="word">${c.amount }</span></td>
							<td width="15%" style="color: #f66;font-weight: bold;"><span class="word">${c.totalPrice }</span></td>
							<td width="15%">
								<span class="word">
									<a href="${pageContext.request.contextPath}/order/fromCart/${c.id }" title="购买">
										<img src="${pageContext.request.contextPath}/style/images/check_ok.png" border="0" style="CURSOR: hand;margin-top: -8px;">
									</a>
								</span>
								<span class="word">
									<a onclick="deletePet(${c.id})" title="删除" style="margin-left: 20px;">
